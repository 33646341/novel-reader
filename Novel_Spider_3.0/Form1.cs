...
namespace Novel_Spider
{
    ...
    {
        IDictionary<string, string> chapters = new Dictionary<string, string>();

        IDictionary<string, string> done = new Dictionary<string, string>();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if(done.Count == 0)
            {
                ...
                {
                    string file_name = Regex.Match(NextMatch.Value, "(?<=\">)([\\S\\s]*?)(?=</a>)").Value; //获取章节名
                    string Aref_Name = Regex.Match(NextMatch.Value, "(?<=<a href =\")([\\S\\s]*?)(?=\">)").Value; //获取章节地址
                    chapters.Add(file_name, Aref_Name);
                }
            }
            else
            {
                foreach (KeyValuePair<string, string> chapter in done)
            ...
            Task task = new Task(() =>
            {
                ...
                foreach (KeyValuePair<string, string> chapter in chapters)
                {
                    if (tag == false)
                        break;
                    string Aref_Name = chapter.Value; //获取章节地址
                    string file_name = chapter.Key;//获取章节名
                    if (done.ContainsKey(file_name))
                        continue;
                    Write_Novel(path + "/" + file_name + ".txt", file_name, Aref_Name);
                    done.Add(file_name, Aref_Name);
                    chapter_num++;
                    download_progress = chapter_num / chapter_sum * 100;
                    MethodInvoker mi = new MethodInvoker(() =>
                    {
                        progressBar1.Value = Convert.ToInt32(download_progress);
                        if(done.Count == chapter_sum)
                        {
                            MessageBox.Show("已完成");
                            this.Close();
                        }
