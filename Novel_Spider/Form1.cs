...
namespace Novel_Spider
{
    public partial class Form1 : Form
    {
        ...
        private void Write_Novel(string filename, string title, string url_name)
        {
            ...
            Regex Rege_Content = new Regex("(?<=false;</script><br />)([\\S\\s]*?)(?<=<br /><script>)");
            string Result_Content = Rege_Content.Match(Content_Html).Value; //获取文章内容

            if (Result_Content == "")
            {
                Novel_type = true;
                Rege_Content = new Regex("(?<=();</script><br />)([\\S\\s]*?)(?<=<br /><script>)");
                Result_Content = Rege_Content.Match(Content_Html).Value; //获取文章内容
            }
